...
// Rockchip pkgconfig (rockchip_mpp) -I/usr/include/rockchip veriyor,
// bu yüzden dosyalar önek olmadan include ediliyor (rk_mpi.h'ın kendi
// içindeki include'larla tutarlı).
#include "mpp_frame.h"
...
struct MppDecoder::Impl {
    ...
    void putPacketLoop() {
        while (running.load()) {
            ...
            // NOT: decode_put_packet dönünce paketi hemen deinit ediyoruz.
            // Rockchip'in kendi örnek kodlarındaki (mpi_dec_test.c tarzı)
            // yaygın pattern bu. "async" ibaresi işlemenin/pipeline'ın
            // asenkron olduğuna işaret ediyor, paket handle'ının çağrı
            // sonrası da canlı kalması gerektiğine dair bir işaret yok.
            // YİNE DE: yüksek hareketli/yüksek bitrate bir akışla test
            // edip bozulma (artefakt) olmadığını doğrula — bu satırın
            // %100 doğruluğunu header'dan teyit edemedim.
    ...
    void handleInfoChange(MppFrame frame) {
        ...
        if (!frm_grp) {
            // DMA32: RGA2'nin 32-bit IOMMU sınırına takılmamak için —
            // bu buffer'ı sonradan RGA'nın kaynak (source) olarak
            // okuyacağını biliyoruz (bkz. 01-IPreprocessor.md).
    ...
    RK_U32 last_logged_fmt = 0xFFFFFFFF;  // sentinel — hiçbir gerçek format bu değeri almaz

    void emitFrame(MppFrame frame) {
        // Format hiç varsayılmıyor — mpp_frame_get_fmt()'in gerçek
        // sonucuna göre hem loglanıyor hem DmaBuffer'a yazılıyor. Log,
        // sadece format DEĞİŞTİĞİNDE basılır (ilk karede + gerçek bir
        // değişiklik olursa) — her karede tekrar etmez.
        ...
        if (base_fmt != last_logged_fmt) {
            std::cout << "[MppDecoder] frame fmt: raw=0x" << std::hex << raw_fmt
                      ...
                      << " (0=NV12/YUV420SP, 5=NV21/YUV420SP_VU, 2=NV16/YUV422SP)"
                      << " FBC=" << (MPP_FRAME_FMT_IS_FBC(raw_fmt) ? "EVET(!)" : "hayır")
                      << "\n";
        ...
        // KRİTİK: MppFrame handle'ı burada capture edilip yaşam döngüsü
        // shared_ptr'a bağlanıyor. Tüketiciler (RGA, ileride) işini
        // bitirip son referans da yok olana kadar mpp_frame_deinit
        // ÇAĞRILMAZ — aksi halde MPP bu fiziksel belleği erken geri
        // alıp bir sonraki kareye yazabilir (bkz. 04-IBufferPool.md §5).
