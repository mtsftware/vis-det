...
struct RtspStreamer::Impl {
    ...
    GstElement* current_appsrc = nullptr;  // media-configure'da doldurulur
...
void onMediaConfigure(GstRTSPMediaFactory* /*factory*/, GstRTSPMedia* media,
    ...
    if (appsrc) {
        ...
        // NOT: burada referansı kasıtlı olarak unref ETMİYORUZ — pushFrame
        // bu handle'ı kullanacak. gst_bin_get_by_name zaten +1 ref veriyor,
        // bu bizim "sahiplendiğimiz" referans oluyor. Basitlik için tek
        // client / paylaşımlı media senaryosunu varsayıyoruz (bkz. header
        // notu — bu M4'e özel, üretim kodu değil).
        impl->current_appsrc = appsrc;
        std::cout << "[RtspStreamer] Client bağlandı, appsrc hazır.\n";
    }
...
bool RtspStreamer::start() {
    ...
    // appsrc: is-live=true + do-timestamp=true => GStreamer PTS'i otomatik
    // atar, biz kendi zaman damgamızı hesaplamak zorunda kalmıyoruz (M4
    // için yeterli; gerçek pipeline'da orijinal decode PTS'i korunmalı).
    std::string launch =
    ...
    g_signal_connect(impl_->factory, "media-configure", G_CALLBACK(onMediaConfigure),
                      impl_.get());
    ...
    if (impl_->source_id == 0) {
        std::cerr << "[RtspStreamer] Server attach başarısız (port kullanımda "
                     "olabilir mi?)\n";
        return false;
    }
    ...
    std::cout << "[RtspStreamer] rtsp://<kart-ip>:" << impl_->port
              << impl_->mount_path << " adresinde yayında.\n";
    return true;
}
...
void RtspStreamer::pushFrame(const DmaBufferPtr& frame) {
    ...
    if (!impl_->current_appsrc || !frame || !frame->virt_addr) {
        return;  // henüz client bağlanmadı — sessizce atla, hata değil
    }

    // TEŞHİS SONUCU (M4): MPP decode çıktısı NV12 (4:2:0) DEĞİL, NV16
    // (4:2:2) — mpp_frame_get_fmt() base=2 (MPP_FMT_YUV422SP) döndürdü.
    // Chroma yatayda yarıya iniyor ama DİKEYDE İNMİYOR. Bu yüzden burada
    // basit bir dikey downsample (her 2 satırdan birini al, nearest-
    // neighbor) ile NV12'ye çeviriyoruz — appsrc caps'i NV12 kalıyor,
    // mpph264enc değişmiyor. Bu geçici/M4'e özel bir çözüm: gerçek
    // pipeline'da bu dönüşüm RGA donanımında (ya da MPP'ye NV12 çıktı
    // zorlayan bir kontrol komutuyla) yapılacak, CPU'da değil — bkz.
    // 01-IPreprocessor.md ve 06-system-architecture.md §9 (açık soru
    // olarak not düşüldü).
    ...
    const size_t nv16_uv_row_bytes = impl_->width;  // W/2 U + W/2 V interleaved
    ...
    if (frame->size < y_size + nv16_uv_row_bytes * impl_->height) {
        return;  // beklenenden küçük buffer — güvenlik kontrolü
    }
    ...
    // Y düzlemi aynen kopyalanır (chroma alt örneklemesi Y'yi etkilemez).
    std::memcpy(map.data, src, y_size);

    // UV düzlemi: NV16'nın her 2 satırından 1'ini alıyoruz (satır 0,2,4,...
    // -> NV12 satır 0,1,2,...). Kaliteli bir ortalama-downsample değil,
    // M4 görsel doğrulaması için yeterli.
    const uint8_t* src_uv = src + y_size;
    uint8_t* dst_uv = map.data + y_size;
    ...
    // gst_app_src_push_buffer buffer'ın sahipliğini ALIR — ekstra
    // gst_buffer_unref çağırmıyoruz.
    GstFlowReturn ret =
