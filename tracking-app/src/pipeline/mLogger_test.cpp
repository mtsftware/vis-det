// mLogger_test: main_m9_test.cpp'nin BİREBİR aynısı (PipelineOrchestrator
// kablolaması, çizim, RTSP çıkışı, --model lighttrack|nanotrack geçişi,
// cloneFrame ile güvenli çizim) — TEK EKLENEN ŞEY: perf_logger_v2.py (RK3588
// Edge AI Workshop) ile aynı CSV şemasını üreten PerfLogger entegrasyonu.
//
// Metrik eşleştirmesi (bkz. PerfLogger.hpp header notu — decode_ms'in bizim
// mimarimizde neden RGA crop süresini taşıdığı orada açıklanıyor):
//   frame_ms    <- ardışık İZLENEN kareler arası duvar saati farkı
//   decode_ms   <- ITracker::lastTiming().crop_us  (RGA girdi hazırlama)
//   invoke_ms   <- ITracker::lastTiming().invoke_us (backbone+head NPU)
//   postproc_ms <- ITracker::lastTiming().postproc_us (CPU decode)
//
// Derleme:
//   g++ -std=c++14 -O2 \
...
//     src/perf/PerfLogger.cpp \
//     src/pipeline/mLogger_test.cpp \
...
//     -lrockchip_mpp -lrga -lpthread -o mlogger_test
//
// Çalıştırma (kartta — npu_core0/1/2 için sudo gerekir, /sys/kernel/debug
// altındaki dosya root-only):
//   sudo ./mlogger_test rtsp://<kaynak_ip>:8554/stream 233,152,109,222 \
//       --model nanotrack --log nanotrack_perf.csv
//
// İzleme (PC'de):
//   ffplay rtsp://<kart_ip>:8557/out
//
// Çıktı: --log ile verilen CSV (varsayılan perf_metrics.csv, flush/100 kare)
// + Ctrl+C ile durdurulduğunda konsola perf_logger_v2.py formatında özet.
...
// main_m8/m9_test.cpp ile aynı çizim (imfillArray, tüm rect'ler 2-hizalı).
void drawBboxOutline(const DmaBufferPtr& frame, const BBox& bbox, int color) {
    ...
    if (!logged_error && r != IM_STATUS_SUCCESS) {
        std::cerr << "[mLogger] drawBboxOutline: imfillArray başarısız: " << imStrError(r)
                  << " (bir kez loglanıyor)\n";
        logged_error = true;
    }
}
...
void printUsage(const char* argv0) {
    std::cerr << "Kullanım: " << argv0
              << " rtsp://kaynak/yol x,y,w,h [--model lighttrack|nanotrack] "
                 "[--log perf.csv] [cikis_port=8557] [width=1280] [height=720] "
                 ...
                 "  --model nanotrack: FP16 NanoTrack, varsayılan yollar "
                 "models/{T_model_backbone,X_model_backbone,model_head}.rknn\n"
                 "  --log <yol>: perf CSV çıktısı (varsayılan perf_metrics.csv)\n";
}
...
int main(int argc, char** argv) {
    ...
    // --model ve --log flag'lerini pozisyonel argümanlardan ayıkla; kalan
    // argümanlar main_m9_test.cpp ile AYNI sırayla işlenir.
    ModelKind model_kind = ModelKind::LightTrackFp16;
    std::string log_path = "perf_metrics.csv";
    std::vector<std::string> positional;
    for (int i = 1; i < argc; ++i) {
        std::string arg = argv[i];
        if (arg == "--model") {
            if (i + 1 >= argc) {
                std::cerr << "[mLogger] --model bir değer bekliyor (lighttrack|nanotrack)\n";
                return 1;
            }
            ...
            if (val == "lighttrack") {
                ...
                std::cerr << "[mLogger] Bilinmeyen --model değeri: " << val
                          << " (beklenen: lighttrack|nanotrack)\n";
                return 1;
            }
        } else if (arg == "--log") {
            if (i + 1 >= argc) {
                std::cerr << "[mLogger] --log bir dosya yolu bekliyor\n";
                return 1;
            }
            log_path = argv[++i];
        } else {
    ...
    if (!parseBbox(positional[1], init_bbox)) {
        std::cerr << "[mLogger] Geçersiz bbox formatı, beklenen: x,y,w,h (pozitif w/h)\n";
        return 1;
    }
    ...
    if (model_kind == ModelKind::LightTrackFp16) {
        ...
        if (!lighttrack->loadModels(template_model, search_model, head_model)) {
            std::cerr << "[mLogger] LightTrack modelleri yüklenemedi\n";
            return 1;
        }
        tracker = lighttrack.get();
        std::cout << "[mLogger] Tracker: LightTrack (FP16)\n";
    } else {
        nanotrack = std::make_unique<NanoTrackImpl>();
        if (!nanotrack->loadModels(template_model, search_model, head_model)) {
            std::cerr << "[mLogger] NanoTrack modelleri yüklenemedi\n";
            return 1;
        }
        tracker = nanotrack.get();
        std::cout << "[mLogger] Tracker: NanoTrack (FP16)\n";
    }
    ...
    if (!streamer.start()) {
        std::cerr << "[mLogger] RTSP streamer başlatılamadı\n";
        return 1;
    }

    PerfLogger::Config perf_cfg;
    perf_cfg.log_path = log_path;
    PerfLogger perf(perf_cfg);
    std::cout << "[mLogger] Perf log: " << log_path << " (flush/100 kare)\n";
    ...
    // frame_ms için: ardışık İZLENEN kareler arası duvar saati farkı
    // (ilk kare 33.3ms — main_video_v2.py'nin "t_prev_frame yoksa 33.3"
    // varsayılanıyla aynı, 30 FPS varsayımı).
    auto last_frame_time = std::chrono::steady_clock::now();
    bool have_last_frame_time = false;

    orchestrator.setResultCallback([&](const FrameResult& r) {
        if (r.tracked) {
            auto now = std::chrono::steady_clock::now();
            double frame_ms =
                have_last_frame_time
                    ? std::chrono::duration<double, std::milli>(now - last_frame_time).count()
                    : 33.3;
            last_frame_time = now;
            have_last_frame_time = true;

            TrackTiming timing = tracker->lastTiming();
            double decode_ms = timing.crop_us / 1000.0;
            double invoke_ms = timing.invoke_us / 1000.0;
            double postproc_ms = timing.postproc_us / 1000.0;
            ...
            // yansıyabilir). Çizim HER ZAMAN bağımsız bir RGA kopyası
            // üzerinde yapılır.
            ...
            std::cerr << "[mLogger] cloneFrame başarısız — bu karede çizim atlandı, "
    ...
    orchestrator.setLostCallback([](uint32_t consecutive) {
        if (consecutive == 1 || consecutive % 150 == 0) {
            std::cout << "[mLogger] hedef kayıp sinyali (ardışık " << consecutive
    ...
    if (!orchestrator.start()) {
        std::cerr << "[mLogger] Orkestratör başlatılamadı\n";
        return 1;
    }

    std::cout << "[mLogger] Pipeline çalışıyor, Ctrl+C ile durdurun...\n";
    ...
    while (g_running.load()) {
        ...
        if (elapsed >= 2.0) {
            ...
            std::cout << "[mLogger] " << ((st.frames_tracked - last_tracked) / elapsed)
                      << " FPS tracking, "
    ...
    perf.flush();
    std::cout << '\n' << perf.summary();
    std::cout << "Metrics → " << log_path << '\n';
    perf.stop();

    std::cout << "[mLogger] Durduruldu.\n";
    return 0;
}