...
// UAPI'leri (Linux 5.6+ stabil) yerel tanımlanıyor (RgaPreprocessor'daki
// M6 pratiğinin devamı; artık tek sahibi burası).
...
struct DmaBufferPool::Impl {
    struct Slot {
        int fd = -1;
        void* virt = nullptr;
        bool in_use = false;          // DEQUEUED..ACQUIRED arası (dolaşımda)
        bool cpu_read_open = false;   // açık DMA_BUF_SYNC START|READ bracket'ı
    };
    struct Pool {
    ...
    // mtx TUTULMADAN çağrılmaz varsayımı yok — dtor'dan da gelir; iç kilit alır.
    void destroyAllLocked() {
        std::lock_guard<std::mutex> lk(mtx);
        for (auto& kv : pools) {
            for (auto& s : kv.second.slots) {
                if (s.in_use) {
                    // shutdown bekledi ama dönmedi — süreci çökertmek yerine
                    // sızıntıyı kabul et (fd kapatılırsa dolaşımdaki kullanıcı
                    // use-after-free yaşar).
                    std::cerr << "[DmaBufferPool] UYARI: '" << kv.first
                              << "' havuzunda iade edilmemiş slot, fd sızdırılıyor\n";
                    continue;
                }
    ...
    int heapFdFor(HeapKind kind) {
        if (kind == HeapKind::CachedDma32) {
            if (heap_fd_cached < 0) {
                ...
                if (heap_fd_cached < 0) {
                    // M6'daki geri-düşüş davranışının aynısı: cached heap bu
                    // imajda yoksa uncached ile devam (doğru ama yavaş).
                    std::cerr << "[DmaBufferPool] UYARI: system-dma32 (cached) yok, "
                                 "uncached heap'e düşülüyor — CPU okumaları yavaş olur\n";
    ...
    // mtx tutulurken çağrılır. true = slot tahsis edildi.
    ...
    // Deleter'dan (herhangi bir thread) çağrılır.
    void releaseSlot(const std::string& pool_name, size_t slot_index) {
        ...
        // Üreticinin açtığı CPU-read bracket'ını iade anında kapat —
        // slotu bir sonraki alan temiz bir durumla başlar.
        if (s.cpu_read_open) {
...
// yaşayan bir buffer'da use-after-free yapmasın diye (bkz. hpp notu).
DmaBufferPtr acquireImpl(const std::shared_ptr<DmaBufferPool::Impl>& impl,
    ...
    // width/height/stride/format/dtype ÜRETİCİ doldurur (04 §9: havuz
    // piksel anlamı bilmez).
    ...
    buf->release_fn = [weak_impl, pool_name, slot_index]() {
        ...
        // Havuz yok olmuşsa: slot kaynakları Impl dtor'unda ele alındı,
        // burada yapılacak güvenli bir şey yok (kasıtlı no-op).
    };
    return buf;
}
...
bool DmaBufferPool::createPool(const std::string& name, uint32_t buffer_size,
    ...
    if (it != impl_->pools.end()) {
        // Idempotent: aynı parametrelerle tekrar isteme sorun değil;
        // FARKLI parametre = programlama hatası, sessizce boyutlandırma yok.
...
void DmaBufferPool::shutdown() {
    {
        ...
        // Dolaşımdaki buffer'ların iadesini bekle (04 sözleşmesi: shutdown
        // tüm fd'leri kapatır — ama dolaşımda buffer varken kapatmak
        // use-after-free olur). Makul bir üst sınır: 2 saniye.
