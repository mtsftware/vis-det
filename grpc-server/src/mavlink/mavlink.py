...
MAVLINK_MSG_ID_TARGET_POINT = 42000
MAVLINK_MSG_ID_BBOX = 42001


class MAVLink_target_point_message(MAVLink_message):
    """
    Target Point Format: X, Y
    """

    id = MAVLINK_MSG_ID_TARGET_POINT
    msgname = "TARGET_POINT"
    fieldnames = ["x", "y"]
    ...
    def __init__(self, x: float, y: float):
        MAVLink_message.__init__(self, MAVLink_target_point_message.id, MAVLink_target_point_message.msgname)
        self._fieldnames = MAVLink_target_point_message.fieldnames
        self._instance_field = MAVLink_target_point_message.instance_field
        self._instance_offset = MAVLink_target_point_message.instance_offset
        self.x = x
        self.y = y
...
setattr(MAVLink_target_point_message, "name", mavlink_msg_deprecated_name_property())


class MAVLink_bbox_message(MAVLink_message):
    """
    BBOX Format: X, Y, W, H
    """

    id = MAVLINK_MSG_ID_BBOX
    msgname = "BBOX"
    fieldnames = ["x", "y", "w", "h"]
    ordered_fieldnames = ["x", "y", "w", "h"]
    ...
    def __init__(self, x: float, y: float, w: float, h: float):
        MAVLink_message.__init__(self, MAVLink_bbox_message.id, MAVLink_bbox_message.msgname)
        self._fieldnames = MAVLink_bbox_message.fieldnames
        self._instance_field = MAVLink_bbox_message.instance_field
        self._instance_offset = MAVLink_bbox_message.instance_offset
        self.x = x
        self.y = y
        self.w = w
        self.h = h
    ...
    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.x, self.y, self.w, self.h), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_bbox_message, "name", mavlink_msg_deprecated_name_property())


mavlink_map: Dict[int, Type[MAVLink_message]] = {
    MAVLINK_MSG_ID_TARGET_POINT: MAVLink_target_point_message,
    MAVLINK_MSG_ID_BBOX: MAVLink_bbox_message,
}
...
class MAVLink(object):
    ...
    def target_point_encode(self, x: float, y: float) -> MAVLink_target_point_message:
        """
        Target Point Format: X, Y
        ...
        """
        return MAVLink_target_point_message(x, y)

    def target_point_send(self, x: float, y: float, force_mavlink1: bool = False) -> None:
        """
        Target Point Format: X, Y
        ...
        """
        self.send(self.target_point_encode(x, y), force_mavlink1=force_mavlink1)

    def bbox_encode(self, x: float, y: float, w: float, h: float) -> MAVLink_bbox_message:
        """
        BBOX Format: X, Y, W, H
        ...
        """
        return MAVLink_bbox_message(x, y, w, h)

    def bbox_send(self, x: float, y: float, w: float, h: float, force_mavlink1: bool = False) -> None:
        """
        BBOX Format: X, Y, W, H
        ...
        """
        self.send(self.bbox_encode(x, y, w, h), force_mavlink1=force_mavlink1)
